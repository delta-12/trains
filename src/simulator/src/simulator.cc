...
{
    tracks_.erase(std::remove_if(tracks_.begin(), tracks_.end(), [track](std::shared_ptr<track_model::TrackModel> model){ return model->GetTrackId() == track; }), tracks_.end());
}
...
types::Error Simulator::AddTrainModel(const types::TrackId track, std::shared_ptr<train_model::TrainModel> train)
{
    ...
    std::vector<std::shared_ptr<track_model::TrackModel>>::iterator i = std::find_if(tracks_.begin(), tracks_.end(), [track](std::shared_ptr<track_model::TrackModel> model){ return model->GetTrackId() == track; });
...
{   
    ...
    if (types::ERROR_NONE == error)
    {
        std::for_each(tracks_.begin(), tracks_.end(), [](std::shared_ptr<track_model::TrackModel> track){ track->Update(); });
    }
