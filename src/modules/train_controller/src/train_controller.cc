...
void SoftwareTrainController::SetOperationMode(const bool operation_mode)
{
    if (operation_mode_ == 0 && operation_mode == 1)
    {
        driver_speed_   = commanded_speed_;
        operation_mode_ = operation_mode; // cannot switch back to auto
    }
}
