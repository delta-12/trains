/*****************************************************************************
* @file ctc_callback_handler.h
*
* @brief Implements the CTC UI callbacks.
*****************************************************************************/
#ifndef TRAINS_SRC_MODULES_WAYSIDE_CONTROLLER_INC_WAYSIDE_CONTROLLER_CALLBACK_HANDLER_H
#define TRAINS_SRC_MODULES_WAYSIDE_CONTROLLER_INC_WAYSIDE_CONTROLLER_CALLBACK_HANDLER_H

#include "channel.h"
#include "launcher.h"
#include "wayside_controller.h"


namespace wayside_controller
{

        void setup_ui(slint::ComponentHandle<ui::WaysideControllerUi> &wayside_controller_ui, wayside_controller::WaysideController &wayside_controller);
        void backend_handler(slint::ComponentHandle<ui::WaysideControllerUi> &wayside_controller_ui);



}

