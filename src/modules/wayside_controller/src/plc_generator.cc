...
{
    public:
        Id(const std::string &id, const wayside_controller::SignalId signal);
        std::string id;
...
static inline void AddId(std::vector<Id> &ids, const std::string &id, const wayside_controller::SignalId signal);
...
Error::Error(parser::ExpressionAstNode node) : node(node)
{
}

Id::Id(const std::string &id, wayside_controller::SignalId signal) : id(id), signal(signal)
{
}
...
std::ostream& operator<<(std::ostream& stream, const Error& error)
{
    ...
    std::visit([&stream](auto &&arg){
            ...
            else if constexpr (std::is_same_v<T, parser::ExpressionAstNode>)
            {
...
static bool AddIfInstructions(parser::SharedIfAstNode node, controller_messages::PlcProgram &plc_program, std::vector<Id> &ids, std::deque<Error> &errors)
{
    ...
    else
    {
        ...
        branch_not_if_instruction->set_argument_0(0);
