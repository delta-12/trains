    ...
    for (int i=1;i<records.size();i++)
    {
        ...
        if (records[i][6]!=" ")
        {
            ...
            else if (temp.substr(0,6)=="SWITCH")
            {
                ...
                else if (stoi(segment)==(stoi(records[i][2])+1)) //if first number is the next block
                {
                    segment=temp.substr(dashpos+1,closep-dashpos-1);
