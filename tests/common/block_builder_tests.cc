...
TEST(BlockBuilderTests, BlueBline)
{
    ...
    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(3, block));
    ASSERT_EQ(block.has_crossing, true);

    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(5, block));
    ASSERT_EQ(block.has_switch, true);

    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(6, block));
    ASSERT_EQ(block.has_switch, true);

    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(10, block));
    ASSERT_EQ(block.station_name, "Station b");
    ASSERT_EQ(block.has_station, true);
    ASSERT_EQ(block.has_light, false);

    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(15, block));
    ASSERT_EQ(block.has_station, true);

    ASSERT_EQ(types::Error::ERROR_INVALID_BLOCK, bb.GetBlock(16, block));
}
...
TEST(BlockBuilderTests, GreenLineV4)
{
    ...
    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(16, block));
    ASSERT_EQ(block.has_station, true);
    ASSERT_EQ(block.station_name, "Downtown");
    ASSERT_EQ(0, block.wayside);

    ASSERT_EQ(types::Error::ERROR_NONE, bb.GetBlock(55, block));
